...
final class ClaudeSessionDiscovery {
    ...
    private static func taskStateFromJsonl(_ file: URL) -> (state: TaskState, lastStopAt: Date?) {
        ...
        // Heuristic 1: file-level mtime. If jsonl is still being written, Claude is active.
        ...
        let recentlyModified: Bool = file.withUnsafeFileSystemRepresentation { cPath in
            ...
            recentModTime = mod
            return Date().timeIntervalSince1970 - mod < 1.0
        }

        // Heuristic 2: entry-level timestamp. Claude writes the full assistant entry
        // (with stop_reason) to jsonl *before* streaming to the terminal. During a long
        // reply the file may not be modified for tens of seconds, but the assistant
        // entry itself is very recent. We treat it as working until the entry ages out.
        let assistantEntryRecent = last.timestamp.map { Date().timeIntervalSince($0) < 8.0 } ?? false

        // Strategy varies by stop_reason:
        //   - nil: actively generating/streaming → longer buffer (8s)
        //   - "tool_use": waiting for tool execution → longer buffer (30s)
        //   - other (end_turn, etc.): turn is complete → only file-write settle (3s)
        let streamingRecent = last.timestamp.map { Date().timeIntervalSince($0) < 8.0 } ?? false
        let toolUseRecent   = last.timestamp.map { Date().timeIntervalSince($0) < 30.0 } ?? false
        ...
        case "assistant":
            if isFinished {
                // Turn complete (e.g. end_turn). No streaming; just wait for file writes.
                state = recentlyModified ? .working : .idle
            } else if last.stopReason != nil {
                // stop_reason == "tool_use". Tool execution may take time.
                state = (recentlyModified || toolUseRecent) ? .working : .idle
            } else {
                // No stop_reason: actively generating. Buffer for long streaming.
                state = (recentlyModified || streamingRecent) ? .working : .idle
            }
        default:
            state = .idle
        }
